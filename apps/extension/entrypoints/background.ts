...
import { saveJob } from "../src/lib/api";
import { RAW_TEXT_CAP } from "@job-analyzer/shared";

// WXT imports this module in Node at build time to generate the manifest, so all
// runtime code must live inside defineBackground's main function. `browser` and
// `defineBackground` are WXT auto-imports (typed via .wxt/).
