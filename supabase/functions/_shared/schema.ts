...
/**
 * Fallback row for when AI parsing fails. Retains raw_text so the parse can be
 * re-run later without re-scraping, and flags parse_status = "needs_review" so
 * the dashboard surfaces it for manual correction. Uses placeholder names
 * because company_name/job_title are NOT NULL with a non-empty CHECK.
 */
export function needsReviewInsert({
  userId,
  sourceUrl,
  rawText,
}: Omit<ExtractionToDbArgs, "parseStatus">): JobInsert {
  return {
    user_id: userId,
